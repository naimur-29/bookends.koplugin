...
function Tokens.expand(format_str, ui, session_elapsed, session_pages_read, preview_mode)
	...
	if needs("d", "D", "n", "w", "a") then
		...
		if needs("d") then
			date_short = os.date("%d %b")
		end
		if needs("D") then
			date_long = os.date("%d %B %Y")
		end
		if needs("n") then
			date_num = os.date("%d/%m/%Y")
		end
		if needs("w") then
			date_weekday = os.date("%A")
		end
		if needs("a") then
			date_weekday_short = os.date("%a")
		end
			...
			if needs("h") then
				if session_pages_read > 0 and chapter_pages_left and chapter_pages_left > 0 then
					...
					if ch_left and ch_left > 0 then
						time_left_chapter = formatDuration((ch_left / pph) * 3600)
					end
				end
			end
			if needs("H") then
				if session_pages_read > 0 and pages_left_book and pages_left_book > 0 then
					...
					if doc_left and doc_left > 0 then
						time_left_doc = formatDuration((doc_left / pph) * 3600)
					end
				end
			end
		end
