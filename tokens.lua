...
local function getDateLocale()
	...
	if _date_locale_cache[lang] ~= nil then
		return _date_locale_cache[lang]
	end
...
function Tokens.expand(format_str, ui, session_elapsed, session_pages_read, preview_mode)
	...
	local function needs(...)
		for i = 1, select("#", ...) do
			if
					format_str:find("%%" .. select(i, ...) .. "[^%a]") or format_str:match("%%" .. select(i, ...) .. "$")
			then
				return true
			end
		end
	...
	if needs("c", "t", "p", "L") then
		...
		if pageno and totalpages then
			local left = tostring(math.max(0, totalpages - pageno))
			if left then
	...
	if has_bar then
		...
		local ticks = {}
		raw_total = bar_doc:getPageCount()
		...
		if is_cre and bar_doc.getCurrentPos and ui.toc then
			...
			if chapter_start then
				...
				if next_chapter then
					...
					end_pos = next_xp and bar_doc:getPosFromXPointer(next_xp)
							or (bar_doc.info and bar_doc.info.doc_height or 0)
				else
	...
	local time_24h = ""
	if needs("k") then
		time_12h = tostring(os.date("%I:%M %p")):gsub("^0", "")
	end
	if needs("K") then
		time_24h = tostring(os.date("%H:%M"))
	end
	...
	if needs("d", "D", "n", "w", "a") then
		...
		if needs("d") then
			date_short = tostring(os.date("%d %b"))
		end
		if needs("D") then
			date_long = tostring(os.date("%d %B %Y"))
		end
		if needs("n") then
			date_num = tostring(os.date("%d/%m/%Y"))
		end
		if needs("w") then
			date_weekday = tostring(os.date("%A"))
		end
		if needs("a") then
			date_weekday_short = tostring(os.date("%a"))
		end
	...
	if needs("q", "Q", "x") and ui.annotation then
		...
		if needs("q") then
			highlights_count = tostring(h or 0)
		end
		if needs("Q") then
			notes_count = tostring(n or 0)
		end
		if needs("x") then
			local bm = 0
