...
function Bookends:_computeTickCache()
	...
	if not doc or not self.ui.toc then
		return {}
	end
	local raw_total = doc:getPageCount()
	if not raw_total or raw_total <= 0 then
		return {}
	end
...
function Bookends:onPageUpdate()
	...
	if self.ui.pagemap and self.ui.pagemap:wantsPageLabels() then
		current = tostring(self.ui.pagemap:getCurrentPageLabel(true)) or ""
	elseif pageno and self.ui.document:hasHiddenFlows() then
		current = tostring(self.ui.document:getPageNumberInFlow(pageno))
	else
		current = tostring(pageno) or "0"
	end
...
function Bookends:onSetDimensions()
	self:markDirty()
end
...
function Bookends:paintTo(bb, x, y)
	...
	for bar_idx, bar_cfg in ipairs(self.progress_bars or {}) do
		if bar_cfg.enabled then
			...
			if bar_w > 0 and bar_h > 0 then
					...
					if is_cre and doc.getCurrentPos and self.ui.toc then
						...
						if chapter_start then
							...
							if next_chapter then
								...
								end_pos = next_xp and doc:getPosFromXPointer(next_xp)
									or (doc.info and doc.info.doc_height or 0)
							else
				...
				OverlayWidget.paintProgressBar(
					bb,
					bar_x,
					bar_y,
					bar_w,
					...
					paint_vertical and "vertical" or nil,
					paint_reverse,
					bar_colors
				)
			end
	...
	for _, pos in ipairs(self.POSITIONS) do
		if self:isPositionActive(pos.key) then
			...
			for i, line in ipairs(pos_settings.lines) do
				...
				if not filter or (filter == "odd" and is_odd_page) or (filter == "even" and not is_odd_page) then
	...
	for key, text in pairs(expanded) do
		...
		for _, i in ipairs(indices) do
			local face_name = (pos_settings.line_font_face and pos_settings.line_font_face[i]) or default_face_name
			local font_size = (pos_settings.line_font_size and pos_settings.line_font_size[i]) or default_font_size
			local style = (pos_settings.line_style and pos_settings.line_style[i]) or "regular"
		...
		local widget, w, h =
			OverlayWidget.buildTextWidget(limited_text, line_configs, pos_def.h_anchor, nil, pos_available_w)
		pre_built[key] = {
			widget = widget,
			w = w,
			h = h,
		...
		for _, rk in ipairs(row_keys) do
			...
			if pb then
				...
				if max_width then
					...
					if pb.widget and pb.widget.free then
						pb.widget:free()
					end
				...
				elseif bar_data[key] then
					...
					widget, w, h =
						OverlayWidget.buildTextWidget(pb.text, pb.line_configs, pb.pos_def.h_anchor, nil, bar_avail)
				else
...
function Bookends:startRefreshTimer()
	if self.refresh_timer_active then
		return
	end
	self.refresh_timer_active = true
	self.refresh_timer_func = function()
		if not self.refresh_timer_active then
			return
		end
		self:markDirty()
	...
	for _, pos in ipairs(self.POSITIONS) do
		table.insert(menu, {
			...
			hold_callback = function(touchmenu_instance)
				if #self.positions[pos.key].lines == 0 then
					return
				end
			...
			return {
				...
				},
				{
					...
					callback = function(touchmenu_instance)
						self:showSpinner(
							_("Truncation gap between regions (px)"),
							self.defaults.overlap_gap,
							0,
							999,
							50,
							function(val)
...
function Bookends:buildSingleBarMenu(bar_idx, bar_cfg)
	...
	return {
		...
		},
		{
			...
			enabled_func = function()
				return bar_cfg.enabled and bar_cfg.type == "book"
			end,
			keep_menu_open = true,
			callback = function(touchmenu_instance)
		...
		},
		{
			...
			keep_menu_open = true,
			callback = function(touchmenu_instance)
				self:showSpinner(_("Bar thickness (px)"), bar_cfg.height or 20, 1, 60, 20, function(val)
					bar_cfg.height = val
					saveBar()
					if touchmenu_instance then
						touchmenu_instance:updateItems()
...
function Bookends:buildBarColorsMenu()
	...
	return {
		...
		},
		{
			...
			checked_func = function()
				return bc.invert_read_ticks ~= false
			end,
			callback = function()
	...
	for i, line in ipairs(lines) do
		table.insert(menu, {
			text_func = function()
				...
				if filter == "odd" then
					tag = " [odd]"
				elseif filter == "even" then
					tag = " [even]"
				end
				local preview = (
					Tokens.expandPreview(
						ps.lines[i] or "",
						self.ui,
						self:getSessionElapsed(),
		...
		preset = {
			...
			positions = {
				tl = {
					lines = { "%T" },
					line_font_size = { [1] = 18 },
					line_style = { [1] = "bolditalic" },
					line_page_filter = { [1] = "even" },
				},
				tc = { lines = {} },
				tr = {
					lines = { "%C" },
	...
	},
	{
		...
		preset = {
			...
			positions = {
				...
				tc = {
					lines = { "\xE2\x8F\xB3 %R \xC2\xBB %s page(s) read this session", "\xEF\x83\xA4 %r page(s)/hr" },
					line_font_size = { [1] = 16 },
					line_style = { [1] = "bold" },
				},
				...
				bc = {
					lines = { "Page %c of %t", "\xEF\x80\xAD %L pages ~ %H left in book" },
					v_offset = 4,
					line_font_size = { [1] = 18 },
					line_style = { [1] = "italic", [2] = "bold" },
					line_v_nudge = { [1] = -14 },
				},
	...
	},
	{
		...
		preset = {
			...
			positions = {
				...
				tc = { lines = {} },
				tr = {
					lines = { "%W  %B%b  \xEF\x82\xA0 %v  \xEE\xA9\x9A %M  \xE2\x98\x80 %f" },
					line_font_size = { [1] = 15 },
				},
				bl = { lines = {} },
...
function Bookends:buildPresetsMenu()
	...
	table.sort(sorted_builtins, function(a, b)
...
function Bookends:editLineString(pos, line_idx)
	...
	local page_filter_button = {
		text_func = function()
			if line_page_filter == "odd" then
				return _("Odd pg")
			elseif line_page_filter == "even" then
				return _("Even pg")
			else
				return _("All pg")
			end
	...
	local function hasBarToken()
		if not format_dialog then
			return current_text:find("%%bar") ~= nil
		end
	...
	local BAR_TYPE_LABELS = {
		chapter = _("Chapter"),
		book = _("Book"),
		book_ticks = _("Book+"),
		book_ticks2 = _("Book++"),
		book_ticks_all = _("Book+++"),
	}
	...
	font_button.callback = function()
		...
		self:showFontPicker(line_face or self:getPositionSetting(pos.key, "font_face"), function(font_filename)
			...
			applyLivePreview()
			format_dialog:reinit()
		end, self:getPositionSetting(pos.key, "font_face"))
	end
		...
		buttons = {
			{
				...
				{
					text_func = function()
						return tostring(pos_settings[field] or 0)
					end,
					enabled = false,
				},
				{
		...
		buttons = {
			{
				...
				{
					text_func = function()
						return self.defaults.font_scale .. "%"
					end,
					enabled = false,
				},
				{
